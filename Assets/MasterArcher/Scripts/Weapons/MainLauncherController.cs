...
public class MainLauncherController : MonoBehaviour {
	...
	private void enemyShoot() {
		...
		// shoot the arrow
		GetComponent<Rigidbody> ().linearVelocity = globalVelocity;

		//add a little wind (stochastic destination) to make the enemy weapon more realistic
		//GetComponent<Rigidbody>().AddForce(WindManager.instance.windForce * Time.fixedDeltaTime, ForceMode.Force);
	}
	...
	void manageArrowRotation() {
		...
		if (GetComponent<Rigidbody> ().linearVelocity != Vector3.zero && !stopUpdate) {
			v = GetComponent<Rigidbody> ().linearVelocity;
